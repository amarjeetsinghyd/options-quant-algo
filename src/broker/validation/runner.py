...
def run_broker_validation() -> Dict[str, Any]:
    ...
    For the default ``ANGEL`` broker the function short‑circuits and returns a
    passing result because no external validation is required.
    """
    ...
    if not script_path or not os.path.isfile(script_path):
        logger.error("No validator script found for broker %s (expected %s)", broker, script_path)
        return {"broker": broker, "status": "FAIL", "details": {"error": "validator not found"}}
